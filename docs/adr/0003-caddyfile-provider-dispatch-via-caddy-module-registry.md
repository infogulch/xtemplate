...
Caddyfile `provider <type> <field> { }` and `controller <type> { }` are JSON adapters only. Modules under `xtemplate.providers.*` / `xtemplate.controller.*` implement `CaddyfileBlockParser` (formerly `CaddyfileProvider`). Dispatch looks up the module by type, asserts the interface, invokes `ParseCaddyfile`, then injects reserved keys (`type`/`name` for providers; `type` for controllers) into `ProvidersRaw` / `ControllerRaw`. This is separate from the provider/controller type registries (ADR-0001, ADR-0007) and doesn't force Caddy into those packages (ADR-0002).
