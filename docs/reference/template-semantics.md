...
`{{define "METHOD /path/{param}"}}` registers a route. Supported methods: `GET`, `POST`, `PUT`, `PATCH`, `DELETE`, and the pseudo-method `SSE` (flushing handler, registered as `GET`). Path parameters use ServeMux syntax and are read with `.Req.PathValue`.
