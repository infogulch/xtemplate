...
func TestModuleUnmarshalJSON(t *testing.T) {
	...
	if len(m.ControllerRaw) == 0 {
		t.Error("ControllerRaw empty, want controller object")
	}
...
// TestServer_ControllerRaw_ViaModuleConfig ensures Caddy-style ControllerRaw is honored
// when Config.Server runs (not only when app.LoadConfig materializes).
func TestServer_ControllerRaw_ViaModuleConfig(t *testing.T) {
	dir := t.TempDir()
	marker := "CADDY-CONTROLLER-RAW"
