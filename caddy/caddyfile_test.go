...
func TestParseCaddyfile_Defaults(t *testing.T) {
	...
	// No controller block → empty ControllerRaw; Server uses DefaultControllerType.
	if len(m.ControllerRaw) != 0 {
		t.Errorf("ControllerRaw = %s, want empty (default via DefaultControllerType at Provision)", m.ControllerRaw)
	}
...
func TestParseCaddyfile_AllOptions(t *testing.T) {
	...
	if len(m.ControllerRaw) == 0 {
		t.Fatal("ControllerRaw empty, want os controller")
	}
	var src struct {
		Type string `json:"type"`
		Path string `json:"path"`
	}
	if err := json.Unmarshal(m.ControllerRaw, &src); err != nil {
		t.Fatal(err)
	}
	if src.Type != "os" || src.Path != "tpl" {
		t.Errorf("controller = %+v, want type=os path=tpl", src)
	}
